...
router.post('/register', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Registration failed. Check the backend database and email configuration.' });
  }
});
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Login failed. Check the backend configuration.' });
  }
});
